...
/*   Updated: 2026/01/02 14:18:24 by vahdekiv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!line_check(line, 'N', 'O') && !map->textures[NORTH])
		map->textures[NORTH] = ft_strdup(line);
	else if (!line_check(line, 'S', 'O') && !map->textures[SOUTH])
		map->textures[SOUTH] = ft_strdup(line);
	else if (!line_check(line, 'E', 'A') && !map->textures[EAST])
		map->textures[EAST] = ft_strdup(line);
	else if (!line_check(line, 'W', 'E') && !map->textures[WEST])
		map->textures[WEST] = ft_strdup(line);
	else if (!line_check(line, 'F', ' ') && !map->textures[FLOOR])
		map->textures[FLOOR] = ft_strdup(line);
	else if (!line_check(line, 'C', ' ') && !map->textures[CEILING])
